...
public:
  T1(const double idelta, const double ichis)
    : delta(idelta), nu((1.0-delta*delta)/4.0), chis(ichis),
      ...
      dvdtNum6((19931218557.0 - 3416878080.0*EulerGamma - 3625933850.0*nu - 6542127900.0*pow(nu,2) - 501270000.0*pow(nu,3) - 2270268000.0*chis*M_PI + 2375049600.0*chis*nu*M_PI + 1117670400.0*pow(M_PI,2) + 179001900.0*nu*pow(M_PI,2))/2.095632e8),
