...
private:
  ...
  double v10a, v10b, v12a, v12b, Phi10a, Phi10b, Phi12a, Phi12b; //tidal terms
  
public:
  T3Tidal(const double delta, const double chis, const double chia, const double iv0, const double love1, const double love2, const double compact1, const double compact2)
    : m1((delta+1.)/2.),
      m2(1.-m1),
      ...
      v10a( -love1*pow((m1/compact1),5)*(-12.+11.*m1)/m1/512. ),
      v10b( -love2*pow((m2/compact2),5)*(-12.+11.*m2)/m2/512. ),
      v12a( love1*pow((m1/compact1),5)*(111339.-4724.*m1-154266.*m1*m1+56280.*m1*m1*m1)/m1/4128768. ),
      v12b( love2*pow((m2/compact2),5)*(111339.-4724.*m2-154266.*m2*m2+56280.*m2*m2*m2)/m2/4128768. ),
      ...
      Phi10a( love1*pow((m1/compact1),5)*3.*(-12.+11.*m1)/m1/512. ),
      Phi10b( love2*pow((m2/compact2),5)*3.*(-12.+11.*m2)/m2/512. ),
      Phi12a( -love1*pow((m1/compact1),5)*5.*(18453.+158.*m1-28110.*m1*m1+10944.*m1*m1*m1)/m1/1376256. ),
      Phi12b( -love2*pow((m2/compact2),5)*5.*(18453.+158.*m2-28110.*m2*m2+10944.*m2*m2*m2)/m2/1376256. )
  ...
  void operator() (double& v, const double t, double& Phi) const {
    ...
    v = 0.5*taum8*(1.0 + taum8*taum8*(v2 + taum8*(v3 + taum8*(v4 + taum8*(v5 + taum8*(v6 + v6lntau*lntau + taum8*(v7 + taum8*taum8*taum8*(v10a+v10b + taum8*taum8*(v12a+v12b) ) ) ) ) ) ) ) );
    Phi = (-1.0/(nu*pow(taum8,5)))*(1.0 + taum8*taum8*(Phi2 + taum8*(Phi3 + taum8*(Phi4 + taum8*(Phi5lntau*lntau + taum8*(Phi6 + Phi6lntau*lntau + taum8*(Phi7 + taum8*taum8*taum8*(Phi10a+Phi10b + taum8*taum8*(Phi12a+Phi12b) )) ) ) ) ) ) );
  }
...
void WU::TaylorT3Tidal(const double delta, const double chis, const double chia, const double v0,
		  vector<double>& t, vector<double>& v, vector<double>& Phi,
		  const double love1, const double love2, const double compact1, const double compact2,
		  const int NPoints)
{
  ...
  T3Tidal d(delta, chis, chia, v0, love1, love2, compact1, compact2);
  ...
  try {
    ...
    cerr << "\nCouldn't find max of v(t) in TaylorT3Tidal.  Assuming it continues to v=" << v1Bad << endl;
    BadPoints = true;
  }
  t.resize(NPoints);
  v.resize(NPoints);
