...
vector<double> AdvLIGO_ZeroDet_HighP(const vector<double>& F, const bool Invert=false, const double NoiseFloor=0.0) {
  ...
  vector<double> LogPSD(WU::Interpolate(ZERO_DET_high_PLogF, ZERO_DET_high_PLogPSD, log(fabs(F))));
...
vector<double> IniLIGO_Approx(const vector<double>& F, const bool Invert=false, const double NoiseFloor=0.0) {
  ...
  if(Invert) {
	...
	PSD[i] = 1.0 / (9e-46 * (pow(4.49*x, -56) + 0.16*pow(x, -4.52) + 0.32*pow(x,2) + 0.52));
	//PSD[i] = 1.0 / (3.316e-46 * (pow(4.49*x, -56) + 0.16*pow(x, -4.52) + 0.32*pow(x,2) + 0.52));
      }
    }
  } else {
    for(unsigned int i=0; i<F.size(); ++i) {
 	...
 	const double x = f / 150.0;
	PSD[i] = 9e-46 * (pow(4.49*x, -56) + 0.16*pow(x, -4.52) + 0.32*pow(x,2) + 0.52);
	//PSD[i] = 3.316e-46 * (pow(4.49*x, -56) + 0.16*pow(x, -4.52) + 0.32*pow(x,2) + 0.52);
      }
    }
  }
  return PSD;
