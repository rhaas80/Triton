#define MAIN_FUNCTION_HAS_PARAMETERS
...
void Output(const string& FileName, const Waveform& a, const unsigned int precision=14) {
...
void Output(const string& FileName, const vector<double>& t, const vector<double>& v, const vector<double>& Phi, const unsigned int precision=14) {
  ofstream ofs(FileName.c_str(), ofstream::out);
  ofs << "# [1] = t" << endl
      << "# [2] = v" << endl
      << "# [3] = Phi" << endl;
...
/// Call this program with 'MismatchContours <q> <chis>', where <q> is the mass ratio and <chis> is the symmetric spin parameter
/// Optionally, use 'MismatchContours <q> <chis> <StartWithMassNumber> <StartWithFreqNumber>' for restarts
...
int main(int argc, char* argv[]) {
  ...
  // Ersatz NR
  const string NRFileName("Input/rhOverM_EOBTotalWaveform_q" + DoubleToString(q) + "_chis" + DoubleToString(chis) + ".dat");
  ...
  const double MinMass=20.0;  // Total binary mass in solar masses
  const double MaxMass=20.0;
  const double MassStep = 0.25;
  const unsigned int NMasses=static_cast<unsigned int>(1.0+(MaxMass-MinMass)/MassStep);
  const double Minomega=0.025;  // Hybridization angular GW frequency (dimensionless)
  const double Maxomega=0.025;
  ...
  // Screen output
  const unsigned int MatchDisplayPrecision=3;
  ...
  Wsc[0] = Waveform("TaylorT1", delta, chis, chia, v0, PNOrder, NRc.LM, nsave);
  Wsc[3] = Waveform("TaylorT4", delta, chis, chia, v0, PNOrder, NRc.LM, nsave);
  Wsc[1] = Waveform("TaylorT2", delta, chis, chia, v0, PNOrder, NRc.LM, max(Wsc[0].T().size(), Wsc[3].T().size()));
  Wsc[2] = Waveform("TaylorT3", delta, chis, chia, v0, PNOrder, NRc.LM, max(Wsc[0].T().size(), Wsc[3].T().size()));
  //#pragma omp parallel for
  for(int n=0; n<NWaveforms; ++n) {
    Wsc[n].OffsetTime(NRPeak22Time-Wsc[n].T().back());
    Wsc[n] = Wsc[n].MatchTo(NRc, RoughMatchT1, RoughMatchT2);
  }
  cout << "☺\n" << endl;
  
  {
    ...
    TaylorT1(delta, chis, chia, v0, t, v, Phi, 3.5, nsave, denseish);
    L=t.size();
    Output("Phasing_T1_Old.dat", t, v, Phi);
    TaylorT4(delta, chis, chia, v0, t, v, Phi, 3.5, nsave, denseish);
    L = (L>t.size() ? L : t.size());
    Output("Phasing_T4_Old.dat", t, v, Phi);
    TaylorT2(delta, chis, chia, v0, t, v, Phi, 3.5, L);
    Output("Phasing_T2_Old.dat", t, v, Phi);
    TaylorT3(delta, chis, chia, v0, t, v, Phi, 3.5, L);
    Output("Phasing_T3_Old.dat", t, v, Phi);
    EOB(delta, chis, chia, v0, t, v, Phi, 3.5, r, prstar, pPhi, nsave, denseish);
    Output("Phasing_EOB_Old.dat", t, v, Phi);
    return 0;
  }
  ...
  cout << "Phase extent of TaylorT1: " << Wsc[0].Phi(0).back()-Wsc[0].Phi(0,0) << endl;
  cout << "Phase extent of TaylorT2: " << Wsc[1].Phi(0).back()-Wsc[1].Phi(0,0) << endl;
  cout << "Phase extent of TaylorT3: " << Wsc[2].Phi(0).back()-Wsc[2].Phi(0,0) << endl;
  cout << "Phase extent of TaylorT4: " << Wsc[3].Phi(0).back()-Wsc[3].Phi(0,0) << endl;
  cout << "Phase extent of EOB: " << NRc.Phi(0).back()-NRc.Phi(0,0) << endl << endl;
  ...
  {
    Output("rhOverM_T1_Old.dat", Wsc[0]);
    Output("rhOverM_T2_Old.dat", Wsc[1]);
    Output("rhOverM_T3_Old.dat", Wsc[2]);
    Output("rhOverM_T4_Old.dat", Wsc[3]);
    Output("rhOverM_EOB_Old.dat", NRc);
    return 0;
  }
  ...
  for(unsigned int o=StartWithFreqNumber; o<omegas.size(); ++o) {
    ...
    #pragma omp parallel for
    for(int n=0; n<NWaveforms; ++n) {
      Ws[n] = Ws[n].MatchToFrequency(NRc, omega, MinAvailableT, MaxAvailableT); // Just match
      Ws[n] = Ws[n].MatchToFrequency(NRc, omega, MinAvailableT, MaxAvailableT, HybridDeltaT); // Now hybridize
    }
    #pragma omp barrier
    ...
    for(unsigned int m=StartWithMassNumber; m<Masses.size(); ++m) {
      ...
      for(int n=0; n<NWaveforms; ++n) {
	Hs[n] = Ws[n];
	Hs[n].SetPhysicalMass(Mass).SetPhysicalDistance(DistanceInMegaparsecs);
      }
      Hs[NWaveforms] = NRc;
      Hs[NWaveforms].SetPhysicalMass(Mass).SetPhysicalDistance(DistanceInMegaparsecs);
	...
	//// Get the InversePSD
	#pragma omp master
	InversePSD = InverseNoiseCurve(Fs[0].Freq(), PSDName);
	#pragma omp barrier
