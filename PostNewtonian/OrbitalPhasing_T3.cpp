...
public:
  T3(const double idelta, const double ichis, const double iv0)
    : delta(idelta), nu((1.0-delta*delta)/4.0), chis(ichis), v0(iv0), Findv0(true),
      ...
      v6((-260539947604339 - 152194575456*pow(chis,2) + 36738273116160*EulerGamma + 579139571772300*nu - 132350549861376*pow(chis,2)*nu - 7948730235600*pow(nu,2) + 42259050763776*pow(chis,2)*pow(nu,2) + 9550175342400*pow(nu,3) - 78457243189248*chis*M_PI + 54017278672896*chis*nu*M_PI + 22592321224704*pow(M_PI,2) - 21170912716800*nu*pow(M_PI,2) + 36738273116160*log(2))/2.884126113792e14),
