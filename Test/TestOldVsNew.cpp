...
using WaveformUtilities::TaylorT1;
using WaveformUtilities::TaylorT2;
using WaveformUtilities::TaylorT3;
using WaveformUtilities::TaylorT4;
using WaveformUtilities::EOB;
...
int main(int argc, char* argv[]) {
  ...
  const unsigned int NMasses=1;//static_cast<unsigned int>(1.0+(MaxMass-MinMass)/MassStep);
  ...
  const unsigned int WindowNCycles=10;
  ...
  const unsigned int nsaveEOB=50;
...

  {
    ...
    L=t.size();
    Output("Phasing_T1_New.dat", t, v, Phi);
    TaylorT4(delta, chis, chia, v0, t, v, Phi, nsave, denseish);
    L = (L>t.size() ? L : t.size());
    Output("Phasing_T4_New.dat", t, v, Phi);
    TaylorT2(delta, chis, chia, v0, t, v, Phi, L);
    Output("Phasing_T2_New.dat", t, v, Phi);
    TaylorT3(delta, chis, chia, v0, t, v, Phi, L);
    Output("Phasing_T3_New.dat", t, v, Phi);
    EOB(delta, chis, chia, v0, t, v, Phi, nsaveEOB, denseish);
    Output("Phasing_EOB_New.dat", t, v, Phi);
    return 0;
  }
  ...
  {
    Output("rhOverM_T1_New.dat", Wsc[0]);
    Output("rhOverM_T2_New.dat", Wsc[1]);
    Output("rhOverM_T3_New.dat", Wsc[2]);
    Output("rhOverM_T4_New.dat", Wsc[3]);
    Output("rhOverM_EOB_New.dat", NRc);
    return 0;
  }
  ...
  for(unsigned int o=StartWithFreqNumber; o<omegas.size(); ++o) {
    ...
    for(unsigned int m=StartWithMassNumber; m<Masses.size(); ++m) {
      ...
      #pragma omp parallel
      {
        #pragma omp for
        for(int n=0; n<NFs; ++n) {
          Fs[n] = WaveformAtAPointFT(WaveformAtAPoint(Hs[n], dt, vartheta, varphi), WindowNCycles);
