% Wo = W1/W2
%
%  This function subtracts two Waveform objects, and
%  normalizes the amplitude of the difference by the
%  amplitude of the second.  The data are interpolated to
%  the largest, finest common time grid.

function Wo = mrdivide(W1,W2)
...
%     if(MaxTime > W2.Time(end)); MaxTime = W2.Time(end); end;
    ...
    Wo.Time = Time;
    Wo.Amp = (AmpI1 - AmpI2) ./ AmpI2;
    Wo.Phi = PhiI1 - PhiI2;
    Wo.TimeOffset = 0;
    Wo.PhaseOffset = zeros(1,size(Wo.Amp,2));
    if(ndims(Wo.Radius)==ndims(W2.Radius) && ...
       sum(size(Wo.Radius)==size(W2.Radius))==2 && ...
       sum(Wo.Radius~=W2.Radius)==0);
        Wo.Radius = W2.Radius;
    else;
        Wo.Radius = -1.0;
    end;
    Wo.Identifier = sprintf('%sD%s',W1.Identifier, ...
                                  W2.Identifier);
    Wo.PostProcessing = ...
        [Wo.PostProcessing; ...
         {sprintf('this = mrdivide(%s, %s);', W1.Identifier, ...
                  W2.Identifier)}; ...
