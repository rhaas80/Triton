...
int main (void) {
  ...
  Waveforms Ws(Radii, DataFile, AreaFile, LapseFile, ADMMass, ChMass);
  Ws.SetCommonTime();
  ...
  for(unsigned int i=0; i<ExtrapolationOrders.size(); ++i) {
    ...
    try {
      ...
      sprintf(ExtrapolatedFile, (Waveform::Types[Extrap.Type] + "_" + ExtrapolatedFiles).c_str(), ExtrapolationOrders[i]);
	...
	if(ConvergenceMatchingT1!=3.0e300 && ConvergenceMatchingT1!=3.0e300) {
	  Diff = Extrap / (Last.MatchTo(Extrap, ConvergenceMatchingT1, ConvergenceMatchingT2));
	} else {
	  Diff = Extrap/Last; // Extrap.L2NormalizedDifference(Last);
	}
	char DifferenceFile[BufferSize];
	sprintf(DifferenceFile, (Waveform::Types[Extrap.Type] + "_" + DifferenceFiles).c_str(), ExtrapolationOrders[i], ExtrapolationOrders[i-1]);
