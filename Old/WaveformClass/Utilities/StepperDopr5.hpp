//// This file is for inclusion in ODEIntegrator.hpp only
...
template <class D>
StepperDopr5<D>::StepperDopr5(std::vector<double> &yy,std::vector<double> &dydxx,double &xx,
			      const double atoll,const double rtoll,bool dens) :
...
void StepperDopr5<D>::step(const double htry,D &derivs) {
  ...
  for (;;) {
    ...
    if (std::abs(h) <= std::abs(x)*EPS) {
      std::cerr << "Stepsize underflow in StepperDopr5"
		<< "\n\tstd::abs(h)=" << std::abs(h)
