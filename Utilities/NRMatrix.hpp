...
namespace WaveformUtilities {
  ...
  public:
    ...
    //inline const T** PtrsPtrs() const { return &DataPtrs[0]; }
    inline T** PtrsPtrs() { return &DataPtrs[0]; }
    inline void RefreshPtr(const unsigned int i);
    void RefreshPtrs();
  ...
  template <class T>
  Matrix<T>::Matrix() : Data(0), DataPtrs(0) { }
  ...
  bool Matrix<T>::operator==(const Matrix<T>& b) const {
    ...
    for(unsigned int row=0; row<nrows(); ++row) {
      if(DataPtrs[row] != b.DataPtrs[row]) { return false; }
  ...
  bool Matrix<T>::operator!=(const Matrix<T>& b) const {
    ...
    for(unsigned int row=0; row<nrows(); ++row) {
      if(DataPtrs[row] != b.DataPtrs[row]) { return true; }
      for(unsigned int col=0; col<ncols(); ++col) {
