...
int main(int argc, char* argv[]) {
  ...
  const double Hybridt1=536.1, Hybridt2=1448.9;
  cout << "Using the NR waveform.\n" << endl;
...
//  const double DetectorSamplingFreq=AdvLIGOSamplingFreq;
  ...
  const string Unique = "_q" + DoubleToString(q) + "_chis" + DoubleToString(chis)
    + "_o" + DoubleToString(OrigStartWithFreqNumber) + "-" + DoubleToString(EndBeforeFreqNumber) + "_NR";
  ...
  Waveform NRc(NRFileName, "MagArg");
  {
    ...
    if(DropOddMModes) { NRRaw.DropOddMModes(); }
    Waveform PN("TaylorT4", delta, chis, chia, v0, NRRaw.LM(), nsave, denseish);
