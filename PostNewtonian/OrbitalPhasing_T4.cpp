...
public:
  T4(const double idelta, const double ichis)
    : delta(idelta), nu((1.0-delta*delta)/4.0), chis(ichis),
      ...
      dvdt6((49341966789 + 26714110500*pow(chis,2) - 6833756160*EulerGamma - 108182476325*nu - 34113687300*pow(chis,2)*nu + 253066275*pow(nu,2) + 8830760400*pow(chis,2)*pow(nu,2) - 906328500*pow(nu,3) - 11176704000*chis*M_PI + 5588352000*chis*nu*M_PI + 2235340800*pow(M_PI,2) + 3938041800*nu*pow(M_PI,2))/4.191264e8),
