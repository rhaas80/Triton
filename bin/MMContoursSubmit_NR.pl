...
`cp /home/boyle/Waveforms/Routines/${Command}_NR /home/boyle/${Command}/`;
...
for $seg (1..$NSegments) {
    ...
    $FinishedStep = "FinishedStep_q${q}_chis${chis}_o${OrigStartFreq}-${OrigEndFreq}_NR";
    if ( -e ${FinishedStep} ) {
    ...
    # Do the qsub
    $command = "echo \"${Command}_NR $q $chis $StartMass $StartFreq $OrigStartFreq $OrigEndFreq\" | qsub -l nodes=1,walltime=5:59:00 -d /home/boyle/${Command} -e q${q}_chis${chis}_${OrigStartFreq}-${OrigEndFreq}.err -o q${q}_chis${chis}_${OrigStartFreq}-${OrigEndFreq}.out -N q${q}_chis${chis}_${OrigStartFreq} -";
    print "> $command\n";
    print `$command`;
}
