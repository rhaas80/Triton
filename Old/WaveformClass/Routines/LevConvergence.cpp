...
int main() {
  ...
  while(A!=string::npos+1) {
	...
	if(ConvergenceMatchingT1!=3.0e300 && ConvergenceMatchingT1!=3.0e300) {
	  Diff[1] = Diff[1].MatchTo(Diff[0], ConvergenceMatchingT1, ConvergenceMatchingT2);
	}
	Diff.AlignPhases();
	sprintf(DiffFile, (Waveform::Types[Diff[0].Type] + "_" + DifferenceFiles).c_str(), LastLev.substr(LastLev.rfind("/")+1).c_str(), NextLev.substr(NextLev.rfind("/")+1).c_str(), ExtrapolationOrders[i]);
